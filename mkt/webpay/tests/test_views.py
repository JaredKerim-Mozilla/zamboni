...
class TestPrepareInApp(InAppPurchaseTest, RestOAuth):
    ...
    def test_get_multiple_jwt_for_same_inapp(self):
        res = self._post()
        eq_(res.status_code, 201, res.content)
        ok_(res.json['webpayJWT'])
        contribution1 = Contribution.objects.get()
        eq_(contribution1.inapp_product, self.inapp)

        res = self._post()
        eq_(res.status_code, 201, res.content)
        ok_(res.json['webpayJWT'])
        contribution2 = Contribution.objects.exclude(pk=contribution1.pk).get()
        eq_(contribution2.inapp_product, self.inapp)
        ok_(contribution1.uuid != contribution2.uuid)
...
class TestStatus(BaseAPI):
    ...
    def test_multiple_inapp_purchases(self):
        inapp = self.get_inapp_product()

        for _ in range(2):
            contribution = self.get_contribution(inapp=inapp)
            data = self.get_status(self.get_contribution_url(contribution))
            eq_(data['status'], 'complete')
            receipt = crack(data['receipt'])[0]
            self.validate_inapp_receipt(receipt, contribution)
